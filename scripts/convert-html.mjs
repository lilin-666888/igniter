...
function rewriteLinks(html, sourceRel) {
  return html.replace(
    /href="([^"]*)"/g,
    (_, href) => `href="${rewriteHref(href, sourceRel)}"`,
  );
}
...
function extractMeta(html) {
  const title =
    html.match(/<title>([^<]*)<\/title>/i)?.[1]?.trim() || "Ceramitell";
  const description =
    html.match(
...
function extractBodyContent(html) {
  ...
  content = content.replace(
    /<div class="topbar">[\s\S]*?<\/div>\s*<\/div>\s*<\/div>\s*/i,
    "",
  );
...
function toVuePage(relPath, html) {
  ...
  const styles = [extractStyles(html), ...inlineStyles]
    .filter(Boolean)
    .join("\n\n");
